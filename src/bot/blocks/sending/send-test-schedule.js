    ...
    scheduleHandler.renderTestScheduleTemplate(studentID)
        .then(({schedules, fileName}) => {
            ...
            setTimeout(() => {
                ...
                if (fileName) {
                    // bot.messageSender.sendText({recipientID: bot.sender.id, content: fileName, typingDelay: 2.5});

                    request({
                        method: 'POST',
                        url: `https://graph.facebook.com/v9.0/me/message_attachments?access_token=${bot.access_token}`,
                        json: {
                            "message":{
                                "attachment":{
                                    "type":"file", 
                                    "payload":{
                                        "is_reusable": true,
                                        url: path.resolve(__dirname, '../../../self_modules/test-schedules-csv/', `${studentID}.csv`),
                                    }
                                }
                            },
